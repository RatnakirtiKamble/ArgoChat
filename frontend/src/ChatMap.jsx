...
const DrawControl = ({ onAreaSelect }) => {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // --- THIS IS THE FIX ---
      // The incorrect `map.hasControl()` check has been removed.
        ...
        <div className="absolute top-3 right-3 z-[1000] flex flex-col items-end">
          ...
          {isPanelOpen && (
            <div className="mt-2 bg-white p-2 rounded-lg shadow-xl flex flex-col space-y-2 border border-gray-200 w-40">
              <button onClick={() => handleSetMode('temperature')} className={`text-sm text-left px-3 py-2 rounded-md font-semibold transition-colors w-full ${heatmapMode === 'temperature' ? 'bg-red-500 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-800'}`}>Temperature</button>
              <button onClick={() => handleSetMode('salinity')} className={`text-sm text-left px-3 py-2 rounded-md font-semibold transition-colors w-full ${heatmapMode === 'salinity' ? 'bg-green-500 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-800'}`}>Salinity</button>
              <button onClick={() => handleSetMode('pressure')} className={`text-sm text-left px-3 py-2 rounded-md font-semibold transition-colors w-full ${heatmapMode === 'pressure' ? 'bg-blue-500 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-800'}`}>Pressure</button>
              <button onClick={() => handleSetMode('fish_prediction')} className={`text-sm text-left px-3 py-2 rounded-md font-semibold transition-colors w-full ${heatmapMode === 'fish_prediction' ? 'bg-yellow-500 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-800'}`}>Fish Prediction 🐟</button>
        ...
        <div className="flex-1 p-4 overflow-y-auto custom-scrollbar bg-gray-100">
          {messages.map((msg, i) => (
            <div key={i} className={`flex my-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              {msg.sender === 'system' ? (<div className="text-center w-full my-2"><span className="text-xs text-gray-500 bg-gray-200 rounded-full px-3 py-1">{msg.text}</span></div>) : (<div className={`rounded-xl p-3 max-w-xs lg:max-w-md shadow-sm ${msg.sender === 'user' ? 'bg-blue-500 text-white' : 'bg-white text-gray-800'}`}><p className="text-sm">{msg.text}</p></div>)}
            </div>
          ))}
          {isLoading && (<div className="flex justify-start"><div className="rounded-xl p-3 max-w-xs bg-white text-gray-800 shadow-sm"><div className="flex items-center space-x-2"><div className="w-2 h-2 bg-gray-400 rounded-full animate-pulse [animation-delay:-0.3s]"></div><div className="w-2 h-2 bg-gray-400 rounded-full animate-pulse [animation-delay:-0.15s]"></div><div className="w-2 h-2 bg-gray-400 rounded-full animate-pulse"></div></div></div></div>)}
          <div ref={chatEndRef} />
        </div>
        <div className="p-4 bg-white border-t border-gray-200">
          <div className="mb-2">
            <button onClick={handleUseCoords} disabled={!selectedBounds} className="text-xs font-semibold text-blue-600 bg-blue-100 hover:bg-blue-200 rounded-full px-4 py-1.5 transition-colors disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed">Use Selected Coordinates</button>
            {selectedBounds && <span className="text-xs text-gray-500 ml-3 hidden sm:inline">{formatBounds(selectedBounds)}</span>}
          </div>
          <form onSubmit={handleSendMessage} className="flex items-center space-x-2">
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask a question..." className="flex-1 w-full px-4 py-2 text-sm bg-gray-100 border border-gray-200 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500" />
